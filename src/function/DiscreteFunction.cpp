  ...
  data_cache(NULL)
{
  ...
  ufc_finite_element_ =
    global_func.finite_element_->create_sub_element(sub_system.array());
  finite_element_ = new FiniteElement(*ufc_finite_element_, true);
  message("SubFunction finite element:"
          + std::string(ufc_finite_element_->signature()));

  // Extract sub dof map and offset
  uint offset = 0;
  ufc::dof_map * ufc_dof_map =
    global_func.dof_map_->create_sub_dof_map(sub_system.array(), offset);
...
void DiscreteFunction::eval(real* values, const real* x) const
{
  ...
  {
    error("Sorry,"
    "point evaluation of functions not implemented for meshes of dimension %d.", gdim);
  }
  Point p;
