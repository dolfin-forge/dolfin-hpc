...
#include <dolfin/fem/%(form_type)s.h>
#include <map>
#include <string>

class %(form_prefix)s : public dolfin::%(form_type)s
{
public:

  %(form_prefix)s(%(constructor_args)s) : dolfin::%(form_type)s(%(base_args)s)
  {
%(constructor_body)s
  }

  %(form_prefix)s(%(map_constructor_args)s) : dolfin::%(form_type)s(%(map_base_args)s)
  {
%(map_constructor_body)s
  }
...
private:
  ...
  // UFC form
  %(class_prefix)s_form_%(class_suffix)s __form;
...
"""
    ...
    for i in range(len(forms)):
        ...
        (coefficient_number, coefficient_name) = generate_coefficient_map_data_hpc(forms[i])
        constructor_args = ", ".join(["dolfin::Function& w%d" % j for j in range(forms[i].num_coefficients)])
        map_constructor_args = "dolfin::Mesh& mesh, std::map<std::string const, dolfin::Function *> const& coefficient_map"
        map_base_args = "mesh"
        if len(constructor_args) == 0:
            ...
            base_args = "w0.mesh()"
        constructor_body = "\n".join(["    __coefficients.push_back(&w%d);" % j for j in range(forms[i].num_coefficients)])
        args = {"form_prefix": form_prefix,
        "form_type": form_type,
		"base_args": base_args,
        "constructor_args": constructor_args,
        "constructor_body": constructor_body,
        "map_base_args": map_base_args,
        "map_constructor_args": map_constructor_args,
        "map_constructor_body": form_map_ctor_body,
        "coefficient_number": coefficient_number,
        "coefficient_name": coefficient_name,
        "class_prefix": prefix.lower(),
        "class_suffix": forms[i].name}
