			   ...
			   std::vector<std::pair<Function*, std::string> > f) const
{
  ...
  for (std::vector<std::pair<Function*, std::string> >::iterator it =
	 f.begin(); it != f.end(); it++)
    {
      Function* u = it->first;
	...
	{
	  if (u->type() == Function::discrete &&
	      u->signature() == FE::DG2DP0S || 
	      u->signature() == FE::DG2DP0V ||
	      u->signature() == FE::DG3DP0S ||
	      u->signature() == FE::DG3DP0V)
	    {
      ...
      if (rank > 1)
	error(
	      "Only scalar and vectors functions can be saved in VTK format.");
      ...
      else
	{
      ...
      if (dim > 3)
	warning(
		"Cannot handle VTK file with number of components > 3. Writing first three components only");
  ...
  for (std::vector<std::pair<Function*, std::string> >::iterator it =
	 f.begin(); it != f.end(); it++)
    {
      ...
      if(!(u->type() == Function::discrete))
	continue;
      else if (!(u->signature() == FE::DG2DP0S) && 
	       !(u->signature() == FE::DG2DP0V) &&
	       !(u->signature() == FE::DG3DP0S) &&
	       !(u->signature() == FE::DG3DP0V))
	{
	...
	error(
      ...
      DofMap const &dmFu = u->dofmap();
      
      real * values = new real[dmFu.dofsmapping_size()];
      GenericVector& Vu   = u->vector();
      Vu.get(values, dmFu.dofsmapping_size(), dmFu.dofsmapping());
      
      uint nbdofspercell_u    = dmFu.local_dimension();
      uint nbnodespercell_u   = nbdofspercell_u / u->dim(0);
...
void VTKFile::pvtuFileWrite(bool mesh_function)
{
  ...
  pvtuFile << "<PDataArray  type=\"UInt32\"  Name=\"connectivity\" />"
	   << std::endl;
  pvtuFile << "<PDataArray  type=\"UInt32\"  Name=\"offsets\" />"
	   << std::endl;
...
void VTKFile::MeshFunctionWrite(T& meshfunction)
{
  ...
  if (meshfunction.dim() != mesh.topology().dim())
    error(
	  "VTK output of mesh functions is implemenetd for cell-based functions only.");
...
void VTKFile::encode_inline_compressed_base64(
					      std::stringstream& stream, const std::vector<T>& data) const
{
  uint32_t header[4];
  header[0] = 1;
