...
//   --it0;
	...
	{
		dolfin::error(
				"Cannot interpolate on zero sample files");
	}
	...
	{
		dolfin::error(
				"At least one of the iteration times used for interpolation is a Nan.");
	}
				      ...
				      real T)
{
  ...
  int num_files = filenames.size();
	...
	if (num_files == 1)
	{
		dolfin::error("Let's divide by zero baby ! Hell yeah ! |m|");
	}
	for (std::vector<std::string>::iterator it = filenames.begin();
			it != filenames.end(); ++it)
	{
	...
	if (counter == 0)
	{
		dolfin::error("Counter irremediably stayed stuck at zero.");
	}
	for (std::map<real, std::string>::const_iterator it = U_files.begin();
			it != U_files.end(); ++it)
	{
		std::cout << std::setw(4) << it->first << " : " << it->second
				<< std::endl;
	}
										...
										std::vector<std::string>& filenames)
{
	...
	if (filenames.size() == 0)
	{
		dolfin::error("Trying to interpolate over zero samples, not smart.");
	}
}
//-----------------------------------------------------------------------------
