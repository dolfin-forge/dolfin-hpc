...
int main(int argc, char** argv)
{
  ...
  Test T(argc, argv);
  {
    ...
    if (mesh.is_distributed())
    {
      ...
      {
        message("Test topological dimension %u", i);
        mesh.init(i);

        DistributedData& distdata = mesh.distdata()[i];

        //
        T.begin("Check ownership");
        {
          ...
          for (uint j = 1; j < pe_size; ++j)
          {
            ...
            MPI_Sendrecv(&sendbuf[dst][0], sendbuf[dst].size(), MPI_UNSIGNED,
                         dst, 0, &recvbuf[0], recvmax, MPI_UNSIGNED, src, 0,
...
*/
      ...
      {
        BoundaryMesh bm(mesh, BoundaryMesh::exterior);
        File f("bme.pvd");
        f << bm;
      }
      T.end();
