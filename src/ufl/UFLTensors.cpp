...
//-----------------------------------------------------------------------------
  ComponentTensor::ComponentTensor(Expression const& expression, MultiIndex const& multi_index) :
...
//-----------------------------------------------------------------------------
  ComponentTensor::ComponentTensor(Expression const& expression, tuple<IndexBase> const& indices,
...
//-----------------------------------------------------------------------------
  std::vector<Class const*> const ComponentTensor::operands(std::string const& name) const
  {
  ...
  tuple<Index> const ComponentTensor::free_indices() const
  {
    ...
    for(dolfin::uint i=0; i<expr_indices.size(); ++i)
      for(dolfin::uint j=0; j<self_indices.size(); ++j)
        if(expr_indices[i] != self_indices[j])
          new_indices.push_back(expr_indices[i]);
...
//-----------------------------------------------------------------------------
  dict<IndexBase, type<dolfin::uint> > const ComponentTensor::index_dimensions() const
  {
    ...
    std::vector<std::pair<IndexBase const *, type<dolfin::uint> const*> > const& 
      dims = expressions_[0]->index_dimensions().map();

    if(f_indices.size() != dims.size())
      error("Size of free indices does not match size of index dimensions.");
...
//-----------------------------------------------------------------------------
  std::vector<Expression const *> const ComponentTensor::fill_expressions(std::vector<repr_t> const& reprs)
  {
...
//-----------------------------------------------------------------------------
  std::vector<Expression const *> const ComponentTensor::fill_expressions(Expression const& e, MultiIndex const& i)
  {
    std::vector<Expression const *> expr;
    expr.push_back(&e);
    expr.push_back(&i);
