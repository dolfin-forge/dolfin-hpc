...
//-----------------------------------------------------------------------------
void LocalMeshCoarsening::coarsenMeshByEdgeCollapse(
		Mesh& mesh, MeshFunction<bool>& cell_marker, bool coarsen_boundary)
{
	...
	while (improving)
	{
				...
				iter != cells_to_coarsen.end(); iter++)
		{
			...
			if (cid != -1)
			{
				mesh_ok = coarsenCell(mesh, coarse_mesh, cid, old2new_vertex,
						old2new_cell, coarsen_boundary);
				if (!mesh_ok)
				{
					warning("Mesh not ok");
				}
...
void LocalMeshCoarsening::collapseEdge(Mesh& mesh, Edge& edge,
										...
										MeshEditor& editor, uint& current_cell)
{
	const CellType& cell_type = mesh.type();
...
//-----------------------------------------------------------------------------
bool LocalMeshCoarsening::coarsenCell(Mesh& mesh, Mesh& coarse_mesh, int cellid,
										Array<int>& old2new_vertex,
										Array<int>& old2new_cell,
										bool coarsen_boundary)
{
	...
	MeshFunction<uint>* bnd_vertex_map = boundary.data().meshFunction(
			"vertex map");
	...
	MeshEditor editor;
	editor.open(coarse_mesh, cell_type.cellType(), mesh.topology().dim(),
			mesh.geometry().dim());
