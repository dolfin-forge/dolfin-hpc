...
To convert from a Medit \texttt{.mesh} format generated by TetGen 
with the -g option type
\begin{code}
# dolfin-convert test_tetgen.mesh mesh.xml
\end{code}
%
To convert from a Gmsh \texttt{.msh} format type 
\begin{code}
# dolfin-convert test_gmsh.msh mesh.xml
\end{code}
In generating a Gmsh mesh, make sure to define a 
physical surface/volume.
%
It is also possible to convert an old \dolfin{} XML (\texttt{.xml}) mesh format to the 
current one by typing
\begin{code}
# dolfin-convert -i xml-old -o xml test_old_xml.xml mesh.xml
\end{code}
%
The \texttt{test\_*} mesh files used in the above examples can be found
in the directory \texttt{src/utils/convert/} of the \dolfin{} source
tree.
