...
//-----------------------------------------------------------------------------
void EquiAffineMapping::map_from_reference_cell(real const * xref, real * x) const
{
  //  J.mult(P, p);
  for (uint i = 0; i < gdim_; ++i)
  {
    x[i] = p0[i];
    for (uint j = 0; j < gdim_; ++j)
    {
...
//-----------------------------------------------------------------------------
void EquiAffineMapping::map_to_reference_cell(real const * x, real * xref) const
{
  //  K.mult(p, P);
  for (uint i = 0; i < gdim_; i++)
  {
...
void EquiAffineMapping::updateTriangle(Cell const& cell)
{
  ...
  std::fill(&p1[0], &p1[d_], 0.0);
  std::fill(&p2[0], &p2[d_], 0.0);
  std::copy(&geom.x(vertices[0])[0], &geom.x(vertices[0])[0] + gdim_, &p0[0]);
  std::copy(&geom.x(vertices[1])[0], &geom.x(vertices[1])[0] + gdim_, &p1[0]);
  std::copy(&geom.x(vertices[2])[0], &geom.x(vertices[2])[0] + gdim_, &p2[0]);
  ...
  // Compute determinant
  det = J[RM(0, 0, d_)] * J[RM(1, 1, d_)] - J[RM(0, 1, d_)] * J[RM(1, 0, d_)];
...
void EquiAffineMapping::updateTetrahedron(Cell const& cell)
{
  ...
  std::copy(&geom.x(vertices[1])[0], &geom.x(vertices[1])[0] + gdim_, &p1[0]);
  std::copy(&geom.x(vertices[2])[0], &geom.x(vertices[2])[0] + gdim_, &p2[0]);
  std::copy(&geom.x(vertices[3])[0], &geom.x(vertices[3])[0] + gdim_, &p3[0]);
  ...
  // Compute determinant
  det = J[RM(0, 0, d_)] * d00 + J[RM(1, 0, d_)] * d10 + J[RM(2, 0, d_)] * d20;
