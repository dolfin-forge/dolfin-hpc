...
void BilinearForm::eval(real block[], const AffineMap& map, real det) const
{
  ...
  const real G0_0 = det*c0*map.g00 + det*c1*map.g01 + det*c2*map.g02;
  const real G0_1 = det*c0*map.g10 + det*c1*map.g11 + det*c2*map.g12;
  const real G0_2 = det*c0*map.g20 + det*c1*map.g21 + det*c2*map.g22;
