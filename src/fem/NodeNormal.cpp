...
void NodeNormal::ComputeNormal(Mesh& mesh)
{
	...
	if (boundary.numCells() > 0)
	{
		for (VertexIterator n(boundary); !n.end(); ++n)
		{
					...
					++boundary_cell)
			{
				...
				Cell mesh_cell(mesh,
						mesh_facet.entities(mesh.topology().dim())[0]);
				...
				{
					n_block[B(irow, j, Nrow)] = mesh_cell.normal(local_facet,
							j);
				}
				irow++;
			}
			...
			if (MPI::numProcesses() > 1)
				if (mesh.distdata().is_shared(id, 0))
				{
					...
					normal_vec[1] +=
							shared_normal[shared_noffset[glb_index] + 1];
					if (nsdim == 3)
						normal_vec[2] += shared_normal[shared_noffset[glb_index]
								+ 2];
			...
			if (nsdim == 3)
			{
				...
				if (n_type == 2)
				{
					...
					{
						real n_norm = sqrt(
								sqr(n1[0]) + sqr(n1[1]) + sqr(n1[2]));
						real nj_norm = sqrt(
								sqr(ns2_block[B(i,0,Nrow)])
										+ sqr(ns2_block[B(i,1,Nrow)])
										+ sqr(ns2_block[B(i,2,Nrow)]));
						alpha = acos(
								(n1[0] * ns2_block[B(i,0,Nrow)]
										+ n1[1] * ns2_block[B(i,1,Nrow)]
										+ n1[2] * ns2_block[B(i,2,Nrow)])
										/ n_norm * nj_norm);
						if (fabs(alpha) > alpha_max)
						{
							n_type = 3;
			...
			else
			{
				...
				send_buff_index[owner].push_back(
						mesh.distdata().get_global(id, 0));
			}
			...
			else
			{
				...
				{
					norm_inv = 1
							/ sqrt(sqr(normal_vec[0] + sqr(normal_vec[1])));
					if (n_type == 2)
					{
				...
				{
					norm_inv = 1
							/ sqrt(sqr(normal_vec[1] + sqr(normal_vec[2])));
					if (n_type == 2)
					{
						tau_vec_1[0] = normal_vec1[0];
						tau_vec_1[1] = normal_vec1[1];
	...
	if (MPI::numProcesses() > 1)
	{
		...
		for (uint i = 1; i < pe_size; i++)
		{
			...
			MPI_Sendrecv(&send_buff_index[dest][0],
					send_buff_index[dest].size(), MPI_UNSIGNED, dest, 0,
					recv_index, recv_count, MPI_UNSIGNED, src, 0,
									...
									MeshFunction<uint> *cell_map)
{
	...
	int recv_size_sh, recv_size_data, recv_size_ndata, recv_count,
			recv_size_area;  // recv_size_ni
	...
	for (int j = 1; j < pe_size; j++)
	{
		...
		for (int i = 0; i < recv_count; i++)
		{
					...
					&& glb_boundary[glb_index])
			{
				...
				shared_normal[shared_noffset[glb_index] + 1] += recv_data[data_i
						+ 1];
				if (nsdim == 3)
					shared_normal[shared_noffset[glb_index] + 2] +=
							recv_data[data_i + 2];
				...
				for (uint k = 0; k < recv_nidx[data_ni]; k++)
					shared_area_block[glb_index].push_back(
							recv_area[aoffset++]);
			}
