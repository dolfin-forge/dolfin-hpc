 ...
 */
  ...
  class FormData // : public Class
  {
    public:
      ...
      dolfin::uint const rank() const;

      dolfin::uint const n_elements() const;
      dolfin::uint const n_cell_elements() const;
      dolfin::uint const n_extf_elements() const;
      dolfin::uint const n_intf_elements() const;

      FiniteElementBase const * get_element(dolfin::uint i) const;

      dolfin::uint const local_finite_element_dimension(dolfin::uint i) const;

      dolfin::uint const n_coefficients() const;
      dolfin::uint const n_cell_coefficients() const;
      dolfin::uint const n_extf_coefficients() const;
      dolfin::uint const n_intf_coefficients() const;

      CoefficientBase const * get_coefficient(dolfin::uint i) const;

      std::vector<std::vector<Class const *> > const& get_cell_operands(dolfin::uint i) const;
      std::vector<std::vector<Class const *> > const& get_extf_operands(dolfin::uint i) const;
      std::vector<std::vector<Class const *> > const& get_intf_operands(dolfin::uint i) const;

      void zero_element_tensor(double* A) const; 

      void compute_element_tensor(double* A, dolfin::UFCReferenceCell const& ref_cell, 
