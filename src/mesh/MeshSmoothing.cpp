...
void MeshSmoothing::smooth_common(Mesh& mesh, MeshSmoothData& smooth_data)
{
  ...
  MPI_Allreduce(&num_un, &max_un, 1, MPI_INTEGER, MPI_MAX,
  ...
  for (uint j = 1; j < pe_size; j++)
  {
    ...
    {
      for (std::vector<double>::iterator iter_vector =
          sender_iterator->second.begin();
  ...
  for (VertexIterator v(mesh); !v.end(); ++v)
  {
    ...
    if (smooth_data.on_boundary_global()(*v)) continue;
    else if (smooth_data.on_boundary()(*v) && pe_size > 1)
    {
      receive_iterator = smooth_data.recv_sum.find(
          mesh.distdata()[0].get_global(v->index()));
  ...
  for (uint j = 1; j < pe_size; j++)
  {
    ...
    if (smooth_data.ghost_tree.size() != 0)
    {
      ...
      {
        for (std::vector<uint>::iterator iter_vector =
            sender_iterator->second.begin();
            iter_vector != sender_iterator->second.end(); iter_vector++)
        {
