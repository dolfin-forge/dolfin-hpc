...
namespace ufl
{
  ...
  //-----------------------------------------------------------------------------
  dolfin::uint const FormData::rank() const
  {
    ...
    if(cell_arguments_.size() != 0)
    {
      ...
      for(dolfin::uint i=0; i<cell_arguments_.size(); ++i)
        if(cell_arguments_[i]->count()>max_count)
          max_count = cell_arguments_[i]->count();
    ...
    if(extf_arguments_.size() != 0)
    {
      ...
      for(dolfin::uint i=0; i<extf_arguments_.size(); ++i)
        if(extf_arguments_[i]->count()>max_count)
          max_count = extf_arguments_[i]->count();
      std::cout << "max count = " << max_count << std::endl;
      return max_count + 1;
    }
    ...
    if(intf_arguments_.size() != 0)
    {
      ...
      for(dolfin::uint i=0; i<intf_arguments_.size(); ++i)
        if(intf_arguments_[i]->count()>max_count)
          max_count = intf_arguments_[i]->count();
      std::cout << "max count = " << max_count << std::endl;
      return max_count + 1;
    }
  ...
  //-----------------------------------------------------------------------------
  dolfin::uint const FormData::n_elements() const
  {
    return cell_elements_.size() + extf_elements_.size() + intf_elements_.size();
  }

  //-----------------------------------------------------------------------------
  dolfin::uint const FormData::n_cell_elements() const
  {
    return cell_elements_.size();
  }

  //-----------------------------------------------------------------------------
  dolfin::uint const FormData::n_extf_elements() const
  {
    return extf_elements_.size();
  }

  //-----------------------------------------------------------------------------
  dolfin::uint const FormData::n_intf_elements() const
  {
  ...
  FiniteElementBase const * FormData::get_element(dolfin::uint i) const
  {
    ...
    if(cell_elements_.size()>i)
      if(cell_elements_[i] != NULL)
        return cell_elements_[i];

    if(extf_elements_.size()>i)
      if(extf_elements_[i] != NULL)
        return extf_elements_[i];
      
    if(intf_elements_.size()>i)
      if(intf_elements_[i] != NULL)
        return intf_elements_[i];

    dolfin::error("Either no element present or index %d is out of range", i);
  }

  //-----------------------------------------------------------------------------
  dolfin::uint const FormData::local_finite_element_dimension(dolfin::uint i) const
  {
  ...
  //-----------------------------------------------------------------------------
  dolfin::uint const FormData::n_coefficients() const
  {
    ...
    return cell_coefficients_.size() + extf_coefficients_.size() + intf_coefficients_.size();
  }

  //-----------------------------------------------------------------------------
  dolfin::uint const FormData::n_cell_coefficients() const
  {
    return cell_coefficients_.size();
  }

  //-----------------------------------------------------------------------------
  dolfin::uint const FormData::n_extf_coefficients() const
  {
    return extf_coefficients_.size();
  }

  //-----------------------------------------------------------------------------
  dolfin::uint const FormData::n_intf_coefficients() const
  {
  ...
  {
    std::cout << "i= " << i << " size= " << cell_coefficients_.size() << std::endl;
    if(cell_coefficients_.size()>i)
      if(cell_coefficients_[i] != NULL)
        return cell_coefficients_[i];

    if(extf_coefficients_.size()>i)
      if(extf_coefficients_[i] != NULL)
        return extf_coefficients_[i];
      
    if(intf_coefficients_.size()>i)
      if(intf_coefficients_[i] != NULL)
        return intf_coefficients_[i];

    dolfin::error("Either no element present or index %d is out of range", i);
  }

  //-----------------------------------------------------------------------------
  std::vector<std::vector<Class const *> > const& FormData::get_cell_operands(dolfin::uint i) const
  {
    return cell_operands_[i];
  }

  //-----------------------------------------------------------------------------
  std::vector<std::vector<Class const *> > const& FormData::get_extf_operands(dolfin::uint i) const
  {
    return extf_operands_[i];
  }

  //-----------------------------------------------------------------------------
  std::vector<std::vector<Class const *> > const& FormData::get_intf_operands(dolfin::uint i) const
  {
  ...
  {
    std::vector<std::vector<std::vector<std::vector<dolfin::real> > > > new_vals0(cell_integrals_.size());
    for(dolfin::uint i=0; i<cell_integrals_.size(); ++i)
    {
        new_vals0[i] = cell_integrals_[i]->evaluate(0, new_vals0[i], ref_cell, q_point, coordinates);
...
//            std::cout << "(" << k << "," << j << ")" << expr[i][k][j]->name() << std::endl;
  ...
  std::vector<Argument const *> const FormData::init_cell_arguments() const
  {
    ...
    {
      std::vector<Class const *> const& arguments
        = cell_integrands_[i]->operands("Argument");
  ...
  std::vector<Argument const *> const FormData::init_extf_arguments() const
  {
    ...
    {
      std::vector<Class const *> const& arguments
        = extf_integrands_[i]->operands("Argument");
      for(dolfin::uint j=0; j<arguments.size(); ++j)
  ...
  std::vector<Argument const *> const FormData::init_intf_arguments() const
  {
    ...
    {
      std::vector<Class const *> const& arguments
        = intf_integrands_[i]->operands("Argument");
      for(dolfin::uint j=0; j<arguments.size(); ++j)
  ...
  std::vector<CoefficientBase const *> const FormData::init_cell_coefficients() const
  {
    ...
    for(dolfin::uint i=0; i<cell_integrands_.size(); ++i)
    {
      std::vector<Class const *> const& coef 
        = cell_integrands_[i]->operands("Coefficient");
      for(dolfin::uint j=0; j<coef.size(); ++j)
        cell_coefficients.push_back(dynamic_cast<Coefficient const *>(coef[j]));
      std::vector<Class const *> const& cnst 
        = cell_integrands_[i]->operands("Constant");
      for(dolfin::uint j=0; j<cnst.size(); ++j)
        cell_coefficients.push_back(dynamic_cast<Constant const *>(cnst[j]));
      std::vector<Class const *> const& vcnst 
        = cell_integrands_[i]->operands("VectorConstant");
      for(dolfin::uint j=0; j<vcnst.size(); ++j)
        cell_coefficients.push_back(dynamic_cast<VectorConstant const *>(vcnst[j]));
      std::vector<Class const *> const& tcnst 
        = cell_integrands_[i]->operands("TensorConstant");
      for(dolfin::uint j=0; j<tcnst.size(); ++j)
        cell_coefficients.push_back(dynamic_cast<TensorConstant const *>(tcnst[j]));
    }
  ...
  std::vector<CoefficientBase const *> const FormData::init_extf_coefficients() const
  {
    ...
    {
      std::vector<Class const *> const& coef 
        = extf_integrands_[i]->operands("Coefficient");
      for(dolfin::uint j=0; j<coef.size(); ++j)
        extf_coefficients.push_back(dynamic_cast<Coefficient const *>(coef[j]));
      std::vector<Class const *> const& cnst
        = extf_integrands_[i]->operands("Constant");
      for(dolfin::uint j=0; j<cnst.size(); ++j)
        extf_coefficients.push_back(dynamic_cast<Constant const *>(cnst[j]));
      std::vector<Class const *> const& vcnst
        = extf_integrands_[i]->operands("VectorConstant");
      for(dolfin::uint j=0; j<vcnst.size(); ++j)
        extf_coefficients.push_back(dynamic_cast<VectorConstant const *>(vcnst[j]));
      std::vector<Class const *> const& tcnst
        = extf_integrands_[i]->operands("TensorConstant");
      for(dolfin::uint j=0; j<tcnst.size(); ++j)
        extf_coefficients.push_back(dynamic_cast<TensorConstant const *>(tcnst[j]));
    }
  ...
  std::vector<CoefficientBase const *> const FormData::init_intf_coefficients() const
  {
    ...
    {
      std::vector<Class const *> const& coef 
        = intf_integrands_[i]->operands("Coefficient");
      for(dolfin::uint j=0; j<coef.size(); ++j)
        intf_coefficients.push_back(dynamic_cast<Coefficient const *>(coef[j]));
      std::vector<Class const *> const& cnst 
        = intf_integrands_[i]->operands("Constant");
      for(dolfin::uint j=0; j<cnst.size(); ++j)
        intf_coefficients.push_back(dynamic_cast<Constant const *>(cnst[j]));
      std::vector<Class const *> const& vcnst 
        = intf_integrands_[i]->operands("VectorConstant");
      for(dolfin::uint j=0; j<vcnst.size(); ++j)
        intf_coefficients.push_back(dynamic_cast<VectorConstant const *>(vcnst[j]));
      std::vector<Class const *> const& tcnst 
        = intf_integrands_[i]->operands("TensorConstant");
      for(dolfin::uint j=0; j<tcnst.size(); ++j)
        intf_coefficients.push_back(dynamic_cast<TensorConstant const *>(tcnst[j]));
    }
  ...
  {
    std::vector<std::vector<std::vector<Class const *> > > expr(cell_integrals_.size());
    for(dolfin::uint i=0; i<cell_integrals_.size(); ++i)
    {
...
//            std::cout << "(" << k << "," << j << ")" << expr[i][k][j]->name() << std::endl;
  ...
  {
    std::vector<std::vector<std::vector<Class const *> > > expr(extf_integrals_.size());
    for(dolfin::uint i=0; i<extf_integrals_.size(); ++i)
    {
...
//            std::cout << "(" << k << "," << j << ")" << expr[i][k][j]->name() << std::endl;
  ...
  {
    std::vector<std::vector<std::vector<Class const *> > > expr(intf_integrals_.size());
    for(dolfin::uint i=0; i<intf_integrals_.size(); ++i)
    {
