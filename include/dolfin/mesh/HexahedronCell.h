...
public:
  ...
  /// Clone pattern
  auto clone() const -> CellType* override { return new HexahedronCell(*this); }
  ...
  auto intersects(MeshEntity const& e, Point const& p1, Point const& p2) const -> bool override;
...
private:
  ...
  auto findFace(uint i, Cell const& cell) const -> uint;

};

//-----------------------------------------------------------------------------
inline auto HexahedronCell::dim() const -> uint
{
  return 3;
}

//-----------------------------------------------------------------------------
inline auto HexahedronCell::num_entities(uint dim) const -> uint
{
...
//-----------------------------------------------------------------------------
inline auto HexahedronCell::num_entities(uint d0, uint d1) const -> uint
{
  dolfin_assert(d0 <= TD);
  dolfin_assert(d1 <= TD);
  return NE[d0][d1];
...
//-----------------------------------------------------------------------------
inline auto HexahedronCell::num_vertices(uint dim) const -> uint
{
  dolfin_assert(dim <= TD);
  return NE[dim][0];
}
...
//-----------------------------------------------------------------------------
inline auto HexahedronCell::orientation(Cell const& cell) const -> uint
{
...
//-----------------------------------------------------------------------------
inline auto HexahedronCell::connectivity_needs_ordering(uint d0, uint d1) const -> bool
{
...
//-----------------------------------------------------------------------------
inline auto HexahedronCell::num_refined_cells() const -> uint
{
  return 8;
}

//-----------------------------------------------------------------------------
inline auto HexahedronCell::num_refined_vertices(uint dim) const -> uint
{
  dolfin_assert(dim <= TD);
  MAYBE_UNUSED(dim);
  return 1;
...
//-----------------------------------------------------------------------------
inline auto HexahedronCell::circumradius(MeshEntity const& entity) const -> real
{
...
//-----------------------------------------------------------------------------
inline auto HexahedronCell::facet_area(Cell const& cell, uint facet) const -> real
{
  dolfin_assert(cell.type() == this->cell_type);
...
//-----------------------------------------------------------------------------
inline auto HexahedronCell::reference_vertex(uint i) const -> real const *
{
  return &VC[i][0];
}
