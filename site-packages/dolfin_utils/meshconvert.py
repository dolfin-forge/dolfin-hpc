        ...
        if line.find("$Elements") == 0:
            ...
            while line.find("$EndElements") == -1:
                ...
                if elem_type == 2:
                    ...
                    if num_tags > 0:
                        tags_2.append(tuple(int(tag)
                                            for tag in element[3:3+num_tags]))
                    dim_2_count += 1
                elif elem_type == 4:
                    if dim < 3:
                        cell_type = "tetrahedron"
...
def triangle2xml(ifilename, ofilename):
    ...
    # Read all the nodes
    nodes = {}
    num_nodes, dim, attr, bound = list(
        map(int, get_next_line(node_file).split()))
    ...
    tris = {}
    num_tris, n_per_tri, attrs = list(
        map(int, get_next_line(ele_file).split()))
    while len(tris) < num_tris:
        tri, n1, n2, n3 = list(map(int, get_next_line(ele_file).split()[:4]))
        tris[tri] = (n1, n2, n3)
...
def scotch_graph2graph_xml(ifilename, ofilename):
    ...
    if not numeric_flag == "000":
        _error(
            "Handling of scotch vertex labels, edge- and vertex weights not implemented")
...
<dolfin xmlns:dolfin="http://www.fenics.org/dolfin/">
  <meshfunction type="uint" dim="%d" size="%d">\n"""
    ...
    for i in range(int(num_cells)):
        ...
        write_cell_tetrahedron(ofile, i, int(
            v[3])-1, int(v[4])-1, int(v[5])-1, int(v[6])-1)
...
class DataHandler(object):
    ...
    """
    State_Invalid, State_Init, State_Vertices, State_Cells, State_MeshFunction = list(
        range(5))
    CellType_Tetrahedron, CellType_Triangle = list(range(2))
...
def _abaqus(ifilename, handler):
    ...
    def read_params(params_spec, pnames, lineno):
        ...
        for p in params_spec:
            ...
            if m is not None:
                ...
                handler.warn(
                    "Invalid parameter syntax on line %d: %s" % (lineno, p))
                continue
            for pn in pnames:
                if pn == pname:
                    params[pn] = val
    ...
    for lineno, l in enumerate(ifile):
        ...
        if m is not None:
            ...
            if sect == "element":
                ...
                if tp not in ("c3d4", "dc3d4"):
                    handler.warn(
                        "Unsupported element type '%s' on line %d" % (tp, lineno))
                ...
                material2elsetids.setdefault(
                    matname, []).append(params["elset"])
            elif sect == "material":
        ...
        if sect == "node":
            ...
            if m is None:
                handler.warn(
                    "Node on line %d is on unsupported format" % (lineno,))
                continue
            idx, c0, c1, c2 = m.groups()
            try:
                coords = [float(c) for c in (c0, c1, c2)]
            except ValueError:
    ...
    for idx, eid in enumerate(elemids):
        ...
        for nid in elem[1:]:
            try:
                elemnodes.append(nodeids.index(nid))
            except ValueError:
                handler.error(
                    "Element %s references non-existent node %s" % (eid, nid))
    ...
    num_entities = 0
    for matname, elsetids in list(material2elsetids.items()):
    ...
    for i, matname in enumerate(materials):
        try:
            elsetids = material2elsetids[matname]
        except KeyError:
...
def exodus2xml(ifilename, ofilename):
    ...
    status, output = getstatusoutput(
        'ncdump '+ifilename + ' > '+netcdffilename)
    if status != 0:
...
def starcd2xml(ifilename, ofilename):
    ...
    for line in lines:
        ...
        write_vertex(ofile, nodenr_map[nodenr], float(
            vertex0), float(vertex1), float(vertex2))
    write_footer_vertices(ofile)
    ...
    for line in lines:
        ...
        if node4 > 0:
            if node2 == node3 and node4 == node5 and node5 == node6 and node6 == node7:  # these nodes should be equal
                ...
                print(("The file does contain cells that are not tetraheders. The cell number is ",
                       cellnr, " the line read was ", line))
        else:
        ...
        if (node4 > 0):
            if node2 == node3 and node4 == node5 and node5 == node6 and node6 == node7:  # these nodes should be equal
                ...
                write_cell_tetrahedron(
                    ofile, counter, nodenr_map[node0], nodenr_map[node1], nodenr_map[node2], nodenr_map[node4])
                counter += 1
