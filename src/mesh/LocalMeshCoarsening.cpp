                                                    ...
                                                    bool coarsen_boundary )
{
  ...
  do {
          ...
          c_it != manager.cells_to_coarsen().end() ; /* do nothing */ )
    {
      ...
      result = coarsenCell(dmesh, manager, *c_it);
...
#ifdef ____USE_D_MESH____
//-----------------------------------------------------------------------------
std::pair<bool,bool> LocalMeshCoarsening::coarsenCell(DMesh& dmesh,
                                                       CoarseningManager& manager,
                                                       uint cell_to_coarsen_id)
{
  ...
  DCell cell_to_coarsen(mesh, manager.cell_map().getCoarseFromFine(cell_to_coarsen_id));
