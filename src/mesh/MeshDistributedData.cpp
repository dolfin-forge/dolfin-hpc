    ...
    const MeshDistributedData& distributed_data)
{
  ...
  if (finalized)
  {
    dolfin_assert(_global_indices_size > 0);dolfin_assert(_global_cell_indices_size > 0);
...
{
  if (MPI::numProcesses() == 1)
    return 0;dolfin_assert( ghost_owner[dim].count(local_index) );
...
{
  if (MPI::numProcesses() == 1)
    return i;dolfin_assert( local_indices[3].count(i) );
  return local_indices[3][i];
}
//-----------------------------------------------------------------------------

MeshGhostIterator::MeshGhostIterator(MeshDistributedData& distdata, uint i) :
    _distdata(distdata)
{
...
//-----------------------------------------------------------------------------
MeshGhostIterator::~MeshGhostIterator()
{
}

//-----------------------------------------------------------------------------
MeshGhostIterator& MeshGhostIterator::operator++()
{
  ++_iter;
  return *this;
}
...
//-----------------------------------------------------------------------------
MeshSharedIterator::MeshSharedIterator(MeshDistributedData& distdata, uint i) :
    _distdata(distdata)
{
...
//-----------------------------------------------------------------------------
MeshSharedIterator::~MeshSharedIterator()
{
}

//-----------------------------------------------------------------------------
MeshSharedIterator& MeshSharedIterator::operator++()
{
  ++_iter;
  return *this;
}
