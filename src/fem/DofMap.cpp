...
DofMap::DofMap(ufc::dof_map& dof_map, Mesh& mesh, bool const dof_map_local) :
		ufc_dof_map_local(dof_map_local), ufc_dof_map(&dof_map), mesh_hash_(
				...
				NULL), local_to_global_(NULL),local_to_global_size_(0), dolfin_mesh(mesh), num_cells(
...
DofMap* DofMap::extractDofMap(const Array<uint>& sub_system, uint& offset) const
{
	...
	message(2, "Extracted dof map for sub system: %s",
...
//-----------------------------------------------------------------------------
ufc::dof_map* DofMap::extractDofMap(const ufc::dof_map& dof_map, uint& offset,
									const Array<uint>& sub_system) const
{
	...
	ufc::dof_map* sub_sub_dof_map = extractDofMap(*sub_dof_map, offset,
			sub_sub_system);
	delete sub_dof_map;
...
//--------------------------------------------------------------------------
// We should use a mutable variable
void DofMap::pretabulate_all_dofs()
{
...
void DofMap::build()
{
	...
	if (MPI::numProcesses() > 1)
	{
		...
		if (ufc_dof_map->global_dimension()
				== dolfin_mesh.distdata().global_numVertices())
		{
			_type_ = 0;
			_local_size = dolfin_mesh.numVertices()
						...
						* dolfin_mesh.distdata().global_numVertices())
		{
			...
			MPI_Exscan(&num_dofs, &offset, 1, MPI_UNSIGNED, MPI_SUM,
					MPI::DOLFIN_COMM);
#else
			MPI_Scan(&num_dofs, &offset, 1, MPI_UNSIGNED, MPI_SUM,
					MPI::DOLFIN_COMM);
			offset -= num_dofs;
#endif
			...
			for (uint j = 1; j < pe_size; j++)
			{
				...
				MPI_Sendrecv(&send_buff[0], send_buff.size(), MPI_UNSIGNED, src,
						2, recv_buff, recv_size, MPI_UNSIGNED, dest, 2,
			...
			for (VertexIterator v(dolfin_mesh); !v.end(); ++v)
				v_map[v->index()] = v_offset[dolfin_mesh.distdata().get_global(
						v->index(), 0)];
		...
		else
		{
			...
			MeshFunction<uint>* cell_map =
					interior_boundary.data().meshFunction("cell map");
			...
			for (CellIterator bc(interior_boundary); !bc.end(); ++bc)
			{
				...
				Cell c(dolfin_mesh,
						f.entities(dolfin_mesh.topology().dim())[0]);
			...
			for (uint k = 1; k < MPI::numProcesses(); ++k)
			{
				...
				MPI_Sendrecv(&send_buffer[0], send_buffer.size(), MPI_UNSIGNED,
						dest, 1, recv_buffer, max_recv, MPI_UNSIGNED, src, 1,
						MPI::DOLFIN_COMM, &status);
				MPI_Get_count(&status, MPI_UNSIGNED, &recv_count);
			...
			uint offset = 0;
#if ( MPI_VERSION > 1 )
			MPI_Exscan(&range, &offset, 1, MPI_UNSIGNED, MPI_SUM,
					MPI::DOLFIN_COMM);
#else
			MPI_Scan(&range, &offset, 1, MPI_UNSIGNED, MPI_SUM,
					MPI::DOLFIN_COMM);
			offset -= range;
#endif
			...
			for (uint k = 1; k < MPI::numProcesses(); ++k)
			{
				...
				for (int i = 0; i < recv_count; i += 2)
				{
					...
					if (forbidden_dofs.find(recv_buffer[i])
							!= forbidden_dofs.end())
					{
...
void DofMap::disp() const
{
	...
	cout << "Geometric dimension:  " << ufc_dof_map->geometric_dimension()
			<< endl;
