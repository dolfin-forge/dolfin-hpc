                         ...
                         uint node, Array<uint>& nodes)
{
  ...
  if ((n_type == 3 && nsdim == 3) || (n_type == 2 && nsdim == 2))
  {
    As.set(zero_block, 1, &nodes[0], static_cast<uint>(a1_ncols),
           a1_indices_array);
    As.set(zero_block, 1, &nodes[1], static_cast<uint>(a1_ncols),
           a1_indices_array);
    if (nsdim == 3) As.set(zero_block, 1, &nodes[2],
                           static_cast<uint>(a1_ncols), a1_indices_array);

    Aset(As, nodes[0], nodes[0], 1.0);
    Aset(As, nodes[1], nodes[1], 1.0);
    if (nsdim == 3) Aset(As, nodes[2], nodes[2], 1.0);

    bset(b, nodes[0], 0.0);
    bset(b, nodes[1], 0.0);
    if (nsdim == 3) bset(b, nodes[2], 0.0);
  }
  else
  {
    ...
    if (nsdim == 3) maxn = fmax(maxn, fabs(n3));
    ...
    if (fabs(fabs(n1) - maxn) < DOLFIN_EPS)
    {       // n1 is a largest component
      r1 = nodes[0];
      r2 = nodes[1];
    }
    else if (fabs(fabs(n2) - maxn) < DOLFIN_EPS)
    {  // n2 is a largest component
      r1 = nodes[1];
      r2 = nodes[0];
    }
    else
    {                         // n3 is a largest component
      if (nsdim == 3)
      {
        r1 = nodes[2];
        r2 = nodes[1];
    ...
    if (nsdim == 3) Aset(As, r1, nodes[2], n3);
    ...
    if (n_type == 2 && nsdim == 3)
    {
      ...
      if (fabs(fabs(nn1) - maxn) < DOLFIN_EPS)
      {       // n1 is a largest component
        r1 = nodes[0];
        r2 = nodes[1];
      }
      else if (fabs(fabs(nn2) - maxn) < DOLFIN_EPS)
      {   // n2 is a largest component
        r1 = nodes[1];
        r2 = nodes[0];
      }
      ...
      if (fabs(fabs(tt1) - maxn) < DOLFIN_EPS)
      {       // tt1 is a largest component
        r2 = nodes[0];
      ...
      else if (fabs(fabs(tt2) - maxn) < DOLFIN_EPS)
      {  // tt2 is a largest component
        r2 = nodes[1];
    ...
    if (n_type == 1 && nsdim == 3)
    {
      ...
      for (uint i = 0; i < a1_ncols; i++)
      {
        if (r2 == a1_indices[i]) ind1 = i;
        if (r3 == a1_indices[i]) ind2 = i;
      }
      ...
      //As.set(row_block, rows, 1, a1_indices_array, a1_ncols);
      As.set(row_block, 1, &row_d1, static_cast<uint>(a1_ncols),
             a1_indices_array);
      ...
      //As.set(row_block, rows, 1, a1_indices_array, a1_ncols);
      As.set(row_block, 1, &row_d2, static_cast<uint>(a1_ncols),
             a1_indices_array);
